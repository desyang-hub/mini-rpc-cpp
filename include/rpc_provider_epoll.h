...
RpcProviderEpoll::RpcProviderEpoll(int port) : _port(port)
{
}

RpcProviderEpoll::~RpcProviderEpoll()
{
}

// 将服务函数进行注册
void RpcProviderEpoll::RegisterMethod(const std::string &svc, const std::string mth, const RpcHandler &handler) {
...
void RpcProviderEpoll::OnConnect(const int epfd, const int sockfd) const {
...
// 处理用户请求
void RpcProviderEpoll::OnMessage(const int sockfd) const {
    // 读取数据， 假如一次读取完成
...
void RpcProviderEpoll::EpollRun() {
    // 1. 创建socket
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd == -1) {
        perror("failed create socket.");
...
void RpcProviderEpoll::OnMessageEvent(const int epfd, const int sockfd) const {
